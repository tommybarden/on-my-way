...
export default function UsersList(props: { users: Record<string, User>; className?: string; }) {
    ...
    const sortedUsers = Object.values(users)
        .filter(user => user.number > 0)
        .sort((a, b) => (a.last_name + a.first_name).localeCompare(b.last_name + b.first_name, 'sv'))
        ...
        <div className={className + ' lg:text-2xl xl:text-3xl'}>
            ...
            <ul role="list" className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 lg:grid-cols-4 gap-6">
                {sortedUsers.map((person) => (
                    <li key={person.id}
                        className="flex flex-col items-center justify-center p-4 bg-gray-800 text-white shadow-md border-l-4 border-gray-200 rounded-r-xl space-y-2">
