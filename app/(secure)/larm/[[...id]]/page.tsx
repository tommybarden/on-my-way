                ...
                {archive &&
                    <div className="md:w-1/2  border p-4">
                        ...
                        <ul role="list" className="divide-y divide-gray-100 py-6">
                            {archive.map((alarm) => (
                                <li key={alarm.id} className={'flex justify-between gap-x-6 ' + ((id ?? latest_alarm.id) == alarm.id ? 'font-bold text-red-800' : '')}>
                                    <Link href={`/larm/${alarm.id}`} className="flex w-full justify-between gap-x-6 py-5">
